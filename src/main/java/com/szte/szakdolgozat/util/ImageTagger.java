...
public class ImageTagger {
    ...
    public List<String> generateTags(byte[] imageBytes) {
            ...
            Tensor inputTensor2 = Tensors.create(new byte[][]{ imageBytes });
            ...
            List<String> topFive = resultList.stream().sorted(Comparator.reverseOrder()).limit(5).map(value -> tagIdNameMap.get(tagIds.get(resultList.indexOf(value)))).toList();
            return topFive;
    }
}
